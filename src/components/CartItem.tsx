...
const CartItem = ({item}: {item: CartItem}) => {
  ...
  const {setQty, removeFromCart} = useStore((store) => store);
  ...
  const onChangeQty = (e:React.ChangeEvent<HTMLSelectElement>) => setQty(item.sku, Number(e.target.value));
  const onRemoveFromCart = () => removeFromCart(item.sku);
...
const MemoizedCartLineItem = memo<typeof CartItem>(CartItem)
